...
<form:form id="article_form" commandName="article" action="${pageContext.request.contextPath}/admin/article/add/${ article.typeString }" method="post">
	...
	<c:if test="${ article.articleId == null }"><h4>添加文章</h4></c:if>
	<c:if test="${ article.articleId != null }"><h4>编辑文章</h4></c:if>
	<div class="box">
		<table>
			<c:if test="${ article.articleId != null }"><tr><td><h5>文章  ID ：</h5></td><td>${ article.articleId }</td></tr></c:if>
	...
	<div class="box" style="text-align:center">
		<c:if test="${ article.articleId == null }"><form:button type="submit">发表文章</form:button></c:if>
		<c:if test="${ article.articleId != null }"><form:button type="submit">发布修改</form:button></c:if>
